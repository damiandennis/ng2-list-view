"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var core_2 = require("@angular/core");
var core_3 = require("@angular/core");
var ListRowComponent = (function () {
    function ListRowComponent() {
        this.previewHidden = true;
        this.visibleEmitter = new core_3.EventEmitter();
        this.dataEmitter = new core_3.EventEmitter();
    }
    ListRowComponent.prototype.togglePreviewVisibility = function () {
        this.previewHidden = !this.previewHidden;
        this.visibleEmitter.emit(this.previewHidden);
    };
    ListRowComponent.prototype.setActiveRow = function (row) {
        if (!this.isActiveRow(row)) {
            this.activeRow = row;
            this.dataEmitter.emit(row);
            if (this.isPreviewHidden()) {
                this.togglePreviewVisibility();
            }
        }
        else {
            this.togglePreviewVisibility();
        }
    };
    ListRowComponent.prototype.isActiveRow = function (row) {
        return this.activeRow === row && !this.isPreviewHidden();
    };
    ListRowComponent.prototype.isPreviewHidden = function () {
        return this.previewHidden;
    };
    return ListRowComponent;
}());
__decorate([
    core_2.Input(),
    __metadata("design:type", Object)
], ListRowComponent.prototype, "previewHidden", void 0);
__decorate([
    core_2.Input(),
    __metadata("design:type", Object)
], ListRowComponent.prototype, "visibleEmitter", void 0);
__decorate([
    core_2.Input(),
    __metadata("design:type", Object)
], ListRowComponent.prototype, "dataEmitter", void 0);
ListRowComponent = __decorate([
    core_1.Component({
        moduleId: module.id,
        selector: "c-list-row",
        templateUrl: "list.row.component.html"
    })
], ListRowComponent);
exports.ListRowComponent = ListRowComponent;
//# sourceMappingURL=list.row.component.js.map